...
/**
 * Azure Managed Redis - Service Principal Authentication (Lettuce)
 * 
 * This example demonstrates how to connect to Azure Managed Redis using
 * a Service Principal (App Registration) with Entra ID authentication.
 * 
 * Service Principal authentication is useful for:
 * - Non-Azure environments (on-premises, other clouds)
 * - CI/CD pipelines
 * - Local development
 * - Any scenario where Managed Identity is not available
 * 
...
public class ServicePrincipalExample {
    ...
    public static void main(String[] args) {
        ...
        if (clientSecret == null || clientSecret.isEmpty()) {
            System.err.println("Error: AZURE_CLIENT_SECRET environment variable is required");
            System.exit(1);
        }
        if (tenantId == null || tenantId.isEmpty()) {
            System.err.println("Error: AZURE_TENANT_ID environment variable is required");
            System.exit(1);
        }
        ...
        System.out.println("\n" + "=".repeat(60));
        System.out.println("AZURE MANAGED REDIS - SERVICE PRINCIPAL AUTH (LETTUCE)");
        System.out.println("Cluster Policy: " + clusterPolicy + (isOSSCluster ? " (cluster-aware)" : " (standard)"));
        System.out.println("=".repeat(60) + "\n");

        if (isOSSCluster) {
            runWithClusterClient(clientId, clientSecret, tenantId, redisHost, redisPort);
        } else {
            runWithStandardClient(clientId, clientSecret, tenantId, redisHost, redisPort);
        }
    }
     ...
     */
    private static void runWithStandardClient(String clientId, String clientSecret, String tenantId, 
                                               String redisHost, int redisPort) {
        TokenBasedRedisCredentialsProvider credentials = null;
        RedisClient redisClient = null;

        try {
            // Create Entra ID credentials provider with Service Principal
            System.out.println("1. Creating credentials provider (Service Principal)...");
            ...
            System.out.println("   ✅ Credentials provider created for client: " + 
            ...
            try (StatefulRedisConnection<String, String> connection = redisClient.connect(StringCodec.UTF8)) {
                ...
                String testKey = "lettuce-sp-test:" + LocalDateTime.now().format(FORMATTER);
                String testValue = "Hello from Lettuce with Service Principal auth!";
     ...
     */
    private static void runWithClusterClient(String clientId, String clientSecret, String tenantId,
                                              String redisHost, int redisPort) {
        TokenBasedRedisCredentialsProvider credentials = null;
        ...
        try {
            ...
            try (StatefulRedisClusterConnection<String, String> connection = clusterClient.connect(StringCodec.UTF8)) {
                ...
                String key1 = "lettuce-sp-shard1:{A}";
                String key2 = "lettuce-sp-shard2:{B}";
                ...
                System.out.println("   " + "-".repeat(50));
                System.out.println("   ✅ MOVED handling verified\n");
