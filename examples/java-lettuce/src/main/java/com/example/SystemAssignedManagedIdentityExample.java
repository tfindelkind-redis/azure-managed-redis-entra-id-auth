...
/**
 * Azure Managed Redis - System-Assigned Managed Identity Authentication (Lettuce)
 * 
 ...
 * 
 * System-Assigned Managed Identity is automatically created and managed by Azure
 * when you enable it on a resource (like a VM or App Service). No AZURE_CLIENT_ID needed.
 * 
 ...
 * 
 * Note: No AZURE_CLIENT_ID needed - Azure automatically provides the identity.
 */
public class SystemAssignedManagedIdentityExample {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("\n" + "=".repeat(60));
        System.out.println("AZURE MANAGED REDIS - SYSTEM-ASSIGNED MANAGED IDENTITY (LETTUCE)");
     ...
     */
    private static void runWithStandardClient(String redisHost, int redisPort) {
        TokenBasedRedisCredentialsProvider credentials = null;
        RedisClient redisClient = null;

        try {
            // Create Entra ID credentials provider with System-Assigned Managed Identity
            System.out.println("1. Creating credentials provider (System-Assigned MI)...");
            ...
            System.out.println("   ✅ Credentials provider created (using VM's system identity)\n");
            ...
            try (StatefulRedisConnection<String, String> connection = redisClient.connect(StringCodec.UTF8)) {
                ...
                String testKey = "lettuce-sysmi-test:" + LocalDateTime.now().format(FORMATTER);
                String testValue = "Hello from Lettuce with System-Assigned MI!";
     ...
     */
    private static void runWithClusterClient(String redisHost, int redisPort) {
        TokenBasedRedisCredentialsProvider credentials = null;
        ...
        try {
            ...
            try (StatefulRedisClusterConnection<String, String> connection = clusterClient.connect(StringCodec.UTF8)) {
                ...
                String key1 = "lettuce-sysmi-shard1:{A}";
                String key2 = "lettuce-sysmi-shard2:{B}";
